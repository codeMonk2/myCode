              ...
              struct options_t *opts)
{
    ...
    while ((c = getopt_long(argc, argv, "i:o:n:sl:", l_opts, &ind)) != -1)
    {
        ...
        case 'n':
            ...
            if (opts->n_threads < 0 || opts->n_threads > INT_MAX)
            {
                 std::cerr << argv[0] << " number of threads needs to be valid" << std::endl;
                 exit(1);
            }
            break;
        case 's':
        ...
        case 'l':
            ...
            if (opts->n_loops < 0 || opts->n_loops > INT_MAX)
            {
                 std::cerr << argv[0] << " number of loops needs to be valid" << std::endl;
                 exit(1);
            }
            break;
        case ':':
