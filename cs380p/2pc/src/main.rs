...
fn spawn_child_and_connect(child_opts: &mut tpcoptions::TPCOptions) -> (Child, Sender<ProtocolMessage>, Receiver<ProtocolMessage>) {
    ...
    let (_, (sender, receiver)): (_, (Sender<ProtocolMessage>, Receiver<ProtocolMessage>)) = server.accept().unwrap();
...
fn connect_to_coordinator(opts: &tpcoptions::TPCOptions) -> (Sender<ProtocolMessage>, Receiver<ProtocolMessage>) {
    ...
    let (to_child,  from_coordinator): (Sender<ProtocolMessage>, Receiver<ProtocolMessage>) = channel().unwrap();
    let (to_coordinator, from_child) : (Sender<ProtocolMessage>, Receiver<ProtocolMessage>) = channel().unwrap();

    let conn = Sender::connect(opts.ipc_path.clone()).unwrap();
    conn.send((to_child,from_child)).unwrap();

    (to_coordinator, from_coordinator)
}
...
fn run(opts: & tpcoptions::TPCOptions, running: Arc<AtomicBool>) {
    ...
    let mut opts_mut = opts.clone();

    let mut coord = coordinator::Coordinator::new(coord_log_path, running);

    opts_mut.mode = "client".to_string();

    for i in 0..opts.num_clients {
        opts_mut.num = i;

        let (_, t, r) = spawn_child_and_connect(&mut opts_mut);
        coord.client_join(i, t, r);
    }

    opts_mut.mode = "participant".to_string();

    for i in 0..opts.num_participants {
        opts_mut.num = i;

        let (_, t, r) = spawn_child_and_connect(&mut opts_mut);
        coord.participant_join(i, t, r);
    }
...
fn run_client(opts: & tpcoptions::TPCOptions, running: Arc<AtomicBool>) {
    let (s,r) = connect_to_coordinator(&opts);

    let mut c = client::Client::new(opts.num.to_string(),
                        s,r, running);

    c.protocol(opts.num_requests);
}
...
fn run_participant(opts: & tpcoptions::TPCOptions, running: Arc<AtomicBool>) {
    ...
    let mut p = participant::Participant::new(opts.num.to_string(),
