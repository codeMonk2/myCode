...
pub struct Participant {
    ...
    to_coord : Sender<ProtocolMessage>,
    from_coord : Receiver<ProtocolMessage>,
    commit_ops : u64,
    abort_ops : u64,
    unknown_ops : u64,
}
...
impl Participant {
    ...
    ///
    pub fn send(&mut self, mut pm: ProtocolMessage, action : bool) {
        let x: f64 = random();
        if action == true {
            pm.mtype = MessageType::ParticipantVoteCommit;
            self.commit_ops += 1;
        } else {
            pm.mtype = MessageType::ParticipantVoteAbort;
            self.abort_ops += 1;
        }
        self.log.append(pm.mtype, pm.txid.clone(), pm.senderid.clone(), pm.opid);
    ...
    pub fn report_status(&mut self) {
        ...
        println!("participant_{}  :\tCommitted: {:6}\tAborted: {:6}\tUnknown: {:6}", self.id_str.clone(), successful_ops, failed_ops, unknown_ops);
    }
    ...
    pub fn wait_for_exit_signal(&mut self) -> bool {
        trace!("Participant-{}::Waiting for exit signal", self.id_str.clone());

		if self.running.load(Ordering::SeqCst) == true {
            return false;
        }

        trace!("Participant-{}::Participant Exiting", self.id_str.clone());

        return true;
    }

    pub fn participant_recv(&mut self) -> Option<ProtocolMessage> {
        let mut m = None;
        if self.running.load(Ordering::SeqCst) != true {
            return None;
        }
                ...
                Some(msg) => {
                    match msg.mtype {
                        MessageType::CoordinatorPropose => {
                            ...
                            let action = self.perform_operation();
                            self.send(msg.clone(), action);
                        },
